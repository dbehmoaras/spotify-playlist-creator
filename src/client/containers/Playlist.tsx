...
function Playlist(props) {
  ...
  const loadPlaylistQString =
    serverRoutes.SRV_LOAD_PLAYLIST +
    "?user=" +
    globalUserId +
    "&playlistId=" +
    activePlaylist.id;
  const removeTrackQString =
  ...
  const deleteSong = async (deleteBody) => {
    return await axios
      .delete(removeTrackQString, { data: deleteBody })
      .then((res) => {
        loadPlaylist().then((list) => {
          setSongs(list.TrackList);
        });
      ...
      return songs.map((ele, idx) => {
        return (
          <div key={idx} id="song-in-list-container">
            <div id="song-in-list">
              <span id="song-span" style={{ fontWeight: 550 }}>
                {ele.Title}
              </span>
              <span id="song-span" style={{ fontStyle: "italic" }}>
                {ele.Artist}
              </span>
            </div>
            <SmallFunctionButton
      ...
      return (
        <div key={0} id="song-in-list-container">
          <div id="song-in-list">
            ...
            <span id="song-span" style={{ fontStyle: "italic" }}>
              Please select a playlist from the component on the left.
            </span>
          </div>
        </div>
      );
